...
class ImageController {
    ...
    public function uploadTableImage() {
        try {
            ...
            // Gerar nome único para o arquivo
            $extension = pathinfo($file['name'], PATHINFO_EXTENSION);
            $filename = 'table_' . $_SESSION['user_id'] . '_' . time() . '.' . $extension;
            $filepath = $this->uploadDir . $filename;
            ...
            $this->database->execute("
                INSERT INTO user_images (user_id, filename, original_name, file_size, mime_type, upload_date, is_table_image)
                VALUES (:user_id, :filename, :original_name, :file_size, :mime_type, :upload_date, :is_table_image)
            ", $imageData);
            ...
            return $this->jsonResponse([
                'success' => true,
                'message' => 'Imagem de mesa enviada com sucesso',
                'image_url' => $imageUrl,
                'filename' => $filename
            ]);
